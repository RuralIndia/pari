...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for face in orm.Face.objects.all().order_by('district', 'created').distinct('district'):
            faces_ = orm.Face.objects.filter(district=face.district)
            if faces_.count() > 1 :
                duplicate_faces.append(faces_)

        for faces_set in duplicate_faces:
            old_face = faces_set[0]
            old_image_collection_id = faces_set[0].image_collection_id

            for face in faces_set[1:]:
                face_img_col_id=face.image_collection_id

                img_col_img = orm['album.ImageCollectionImage'].objects.get(image_collection=face.image_collection)
                img_col_img.image_collection_id = old_image_collection_id
                img_col_img.save()

                orm.FaceImage.objects.filter(face_id=face.pk).update(face=old_face)

                face.delete()
                face.save()

                orm['album.ImageCollection'].objects.filter(pk=face_img_col_id).delete()
    ...
    models = {
        ...
        u'faces.face': {
            ...
            'created': ('django.db.models.fields.DateTimeField', [], {'null': 'True'}),
            'description': ('django.db.models.fields.TextField', [], {'blank': 'True'}),
            'district': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['faces.District']", 'null': 'True'}),
            'expiry_date': ('django.db.models.fields.DateTimeField', [], {'null': 'True', 'blank': 'True'}),
            'gen_description': ('django.db.models.fields.BooleanField', [], {'default': 'True'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
