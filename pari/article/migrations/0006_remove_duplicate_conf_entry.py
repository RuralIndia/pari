...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        dup_ids= db.execute("select id from conf_setting where name = 'RATINGS_ACCOUNT_REQUIRED' order by id")
        if len(dup_ids) > 1:
            db.execute("delete from conf_setting where id = %s", [dup_ids[0][0]])
