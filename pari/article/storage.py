...
class DropboxStorage(StorageMixin, Storage):
    """
    A storage class providing access to resources in a Dropbox Public folder.
    """

    def __init__(self, location=settings.MEDIA_ROOT):
        session = DropboxSession(settings.DROPBOX_CONSUMER_KEY, settings.DROPBOX_CONSUMER_SECRET, "app_folder", locale=None)
    ...
    def isdir(self, name):
        response = self.client.metadata(name)
        return response['is_dir']

    def isfile(self, name):
        response = self.client.metadata(name)
        return not response['is_dir']

    def move(self, old_file_name, new_file_name, allow_overwrite=False):
        self.client.file_move(old_file_name, new_file_name)

    def makedirs(self, name):
        self.client.file_create_folder(name)
